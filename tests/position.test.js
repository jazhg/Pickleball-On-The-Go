...
test('calibrates, maps sideways/depth motion, and freezes on missing landmarks', () => {
  ...
  assert.equal(tracker.update(body()).court_y, CONFIG.player.z);
  const moved = tracker.update(body(0.6, 0.25));
  assert.ok(moved.court_x < 0, 'mirrored lateral movement');
  assert.ok(moved.court_y < CONFIG.player.z, 'larger shoulders mean closer to net');
  ...
  assert.ok(tracker.position.z <= CONFIG.tracking.maxZ);
